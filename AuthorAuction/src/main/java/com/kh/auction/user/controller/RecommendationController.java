...
//		String savePath = "/Users/kimgahyun/uploadFiles"; 				// 맥
		...
		System.out.println("DDDDDDDDDDDDD"+originFileName);
...
System.out.println("r"+r);
	...
	public String updateReply(@RequestParam(value="page", defaultValue="1") int page, 
		...
		int updateAttmResult = 0;		// 업데이트 함
		int deleteAttmResult = 0;		// 삭제할
		
		HashMap<String, Object> map = new HashMap<String, Object>();
		
		// 파일 선택 먼저 확인
		if(file != null && !file.isEmpty()) {		// 파일이 O
			String rename = saveFile(file);
			map.put("recNo", r.getRecNo());
			map.put("rename", rename);
			updateAttmResult = rService.updateAttmReply(map);
		} 
		
		if(deleteAttm != null && !deleteAttm.equals("none")) {
			String[] split = deleteAttm.split("/");
			String delRename = split[0];
			deleteAttmResult = rService.deleteAttmReply(delRename);
